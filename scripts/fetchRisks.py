...
class fetchData:
    # subscribedItem = ReadConfig.getSubscribedAPI()
    legalRegister = apiLegEndpoints.getLegalRegister()
    # riskRegister = ReadConfig.getRiskAPI()
    # issueManagement = ReadConfig.getIssueAPI()

    def fetchAPIresponse(self, username, password):
        token = getToken.getAccessToken(username, password)
        responseFromRequest = getResponseHttps.request(self.legalRegister, token)  # request is utility which we use for get response from API, and getResponseHttps is file name
        # print(result)
        myjson, statusCode = responseFromRequest
        if statusCode == 200:
            print("\033[92mYour API request is successfully completed: response code is \033[0m", "\033[95m"+str(statusCode)+"\033[0m")
        else:
            print("\033[91mYour API request aborted (server error):\033[0m")
        # print(statusCode)
        req_data_list = []
        for x in myjson["records"]:
            json_data = x["orgLegislation"]["legislation"]["legActId"]
